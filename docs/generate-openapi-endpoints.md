...
EDIParse is a REST-API application based on Python's FastAPI framework that uses the EDIFACT Parser library for parsing
EDIFACT messages. The API endpoints of the application are generated by OpenAPI Generator using the provided OpenAPI
YAML specification.

The main OpenAPI specification of the application is described in this file:
[docs/ediparse.openapi.yaml](ediparse.openapi.yaml).
...
### Generate the EDIParse API Server
...
docker run --rm \
  ...
  -i /local/docs/ediparse.openapi.yaml \
  -g python-fastapi \
  -o /local
```
...
3. Implement the business logic for each endpoint
   - Currently, all implementations are within this folder: [impl](../src/ediparse/adapters/inbound/rest/impl), which is also git-committed.
...
> [**edifact_parser_api.py**](../src/ediparse/adapters/inbound/rest/apis/edifact_parser_api.py) as well as the
...
> [**ediparse.openapi.yaml**](ediparse.openapi.yaml),
> the [**edifact_parser_api.py**](../src/ediparse/adapters/inbound/rest/apis/edifact_parser_api.py) file needs to be
